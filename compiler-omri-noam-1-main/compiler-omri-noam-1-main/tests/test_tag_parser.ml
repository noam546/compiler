 ...
 {name = "If";
  ...
  expected =
   ScmIf (ScmApplic (ScmVar "a", []), ScmConst (ScmString "then"),
    ...
    {name = "if a b c";
      input= ScmPair (ScmSymbol "if", ScmPair (ScmSymbol "a", ScmPair (ScmSymbol "b", ScmPair (ScmSymbol "c", ScmNil))));
      expected = ScmIf (ScmVar "a", ScmVar "b", ScmVar "c")};
 {name = "Seq";
  input =
   ScmPair
    (ScmSymbol "begin",
   ...
   ScmPair
    (ScmSymbol "quasiquote",
     ScmPair(ScmPair (ScmSymbol "unquote",
                      ScmPair (ScmSymbol "a", ScmNil)), ScmNil));
  expected = ScmVar "a"};
 {name = "QQ-unquote-splicing";
  input =
   ScmPair(ScmSymbol "quasiquote",
          ScmPair(ScmPair(ScmSymbol "unquote-splicing",
                           ScmPair(ScmPair(ScmSymbol "+",
                                           ScmPair (ScmNumber (ScmRational (1, 1)),
