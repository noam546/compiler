...
%define MAKE_LITERAL_SYMBOL(add) MAKE_LITERAL T_SYMBOL ,dq add
...
%macro CHANGE_STACK_TO_LAMBDA_OPT 1 ;; %1 - number of args without the optional
    push rbp
    mov r14,rbp
    mov rbp,rsp
    mov rcx,PARAM_COUNT ;; all params with opt
    mov r8,%1 ;;real num of params
    sub rcx,r8
    mov r9,SOB_NIL_ADDRESS
    cmp rcx,0
    jne %%create_list_loop
    mov rbx,SOB_NIL_ADDRESS
    %%create_list_loop:
        cmp rcx,0
        je %%end_create_list_loop
        mov rdx,qword[rbp+8*(%1+3+rcx)]
        MAKE_PAIR(rbx,rdx,r9)
        mov r9, rbx
        dec rcx
        jmp %%create_list_loop
    %%end_create_list_loop:
    %%create_simple_list:
        xor r11,r11
        mov r11,r8
        shl r11,3
        ...
        inc rcx
        jmp %%start_loop_param
        %%end_loop_param:
    mov rsi,[rbp+8] ;ret
    mov rdx,[rbp+16] ;env
    mov rdi,[rbp+24] ; old param count
    add rsp,32 ;clean basic
    lea rcx, [rdi*8]
    add rsp,rcx ;;clean all params
    push rbx ;;push the list of opt params
    mov r11,r8
    cmp r11,0
    je %%pushing_env_ret_num
    mov rcx,r11
    dec rcx
    %%pushing_old_params:
        cmp rcx,0
        je %%end_pushing_old_params
        push qword [r10+(8*rcx)]
        dec rcx
        jmp %%pushing_old_params
    %%end_pushing_old_params:
    push qword [r10] ;;pushing last element
    %%pushing_env_ret_num:
        inc r8
        push r8 ;; push num of params
        push rdx ;;push env
        push rsi ;; push ret
%endmacro
...
%macro SHIFT_FRAME 1
	...
	mov rax, PARAM_COUNT
	add rax, 3
	shl rax, 3  ;;for bytes *8
	mov rdx, %1
	mov rcx, 1
	lea rsi, [rbp + rax] ;;creating the pointer to the new stack
%%start_shift_loop:
	cmp rcx, rdx
	je %%end_shift_loop
	lea r9, [rbp]
	...
	inc rcx
	sub rsi, 8
	jmp %%start_shift_loop
%%end_shift_loop:
	pop rbx  ;;old rbp
	pop rax
	lea rsp, [rsi+8]
	mov rbp, rbx ;re enter rbp
%endmacro

%define MAKE_LITERAL_SYMBOL(add) \
        MAKE_LITERAL T_SYMBOL,dq add
